...
public class VoucherActivity extends BaseActivity {
	...
	public void closePressed(View v) {
		int pos = listViewVoucher.getPositionForView(v);
		/*sqliteHelper.openDataBase();
		sqliteHelper.deleteVoucher(voucherList.get(pos));
		sqliteHelper.close();
		voucherList.remove(pos);*/
		
		try {
			voucherList = (ArrayList<Voucher>) ObjectSerializer.deserialize(pref.getString("Voucher", ObjectSerializer.serialize(new ArrayList<Voucher>())));
			voucherList.remove(voucherList.get(pos));
			pref.edit().putString("Voucher", ObjectSerializer.serialize(voucherList));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
